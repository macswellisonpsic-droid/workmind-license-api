...
async function sendLicenseEmail({
  ...
  const safeEmail = escapeHtml(customerEmail || to || '');
    ...
    <div style="font-family: Arial, sans-serif; max-width: 680px; margin: 0 auto; color: #1f2937;">
      ...
      <div style="border: 1px solid #e5e7eb; border-top: 0; padding: 28px; border-radius: 0 0 18px 18px;">
        ...
        <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 14px; padding: 18px; margin: 22px 0;">
          <p style="margin: 0 0 8px;"><strong>E-mail de ativação:</strong> ${safeEmail}</p>
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to,
      reply_to: process.env.LICENSE_REPLY_TO || process.env.LICENSE_FROM_EMAIL,
  ...
  if (response.status === 409 && cleanIdempotencyKey) {
    console.warn('[EMAIL] Envio duplicado bloqueado por idempotência:', {
  ...
  if (!response.ok) {
    throw new Error(`Erro ao enviar e-mail pelo Resend: ${JSON.stringify(data)}`);
  }
